...
17. ~~Native Word cross-references~~ ✓ — cross-references render as internal hyperlinks (`<w:hyperlink w:anchor="...">`) with bookmarks at anchor targets. Ctrl+click navigation in Word. No field codes or ZIP post-processing needed — uses docx-rs's native `Hyperlink` + `BookmarkStart`/`BookmarkEnd` API. Note: auto-updating clause numbers is not possible since numbers come from Word's numbering engine, but clickable navigation works.
