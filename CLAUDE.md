...
cargo run -- lint lexicon/example.md --fix           # normalise hand-numbered ordinals in place
...
| `src/fix.rs` | Autofix — normalises hand-numbered ordinals to `1.`, dedenting item content to match; verified by structural fingerprint before writing |
...
- **Linter architecture** — spec-compliance checks live in the parse/resolve pipeline (so `build` sees them too); lint-only checks that would be noise during a build (metadata completeness info, exhibit file existence) live in `src/lint.rs`. Every `Diagnostic` carries a severity (`Error`/`Warning`/`Info`), a stable kebab-case rule code (e.g. `unused-term`), a human-readable location, and a 1-based source line/column where known (comrak `sourcepos` + front-matter line offset threaded through the parser as `Clause.source_pos`/`Addendum.source_pos`, type `error::SourcePos`). All rule codes live in the `RULES` registry in `src/lint.rs` (`lint --list-rules`); config, flags, and suppressions are validated against it (`unknown-lint-rule`). `Info` diagnostics are lint-only: `print_diagnostics` in `main.rs` skips them during `build`. Exit codes: 0 clean, 1 errors (or warnings with `--strict`), 2 unreadable input or config. The usage scan for `unused-term` excludes bold text (bold marks definition sites, not references, per spec 4.1) and treats terms with 2+ definition sites as used (the `duplicate-definition` warning covers those).
- **Continuation indent is marker-derived, not level-derived** — a clause's content column is `marker_col + NodeList.padding` (comrak's `padding` covers the marker literal plus trailing spaces: 3 for `1. `, 4 for `10. `, 5 for `100. `). The old spec formula `(4 × level) + 3` hard-coded a single-digit marker; helpers in `parser/clause.rs` (`item_content_col`, `open_item_chain`) compute it from the source instead. Two failure modes, two rules: a block falling 4+ short becomes a CommonMark *indented code block* and is dropped — `continuation-indent` (error), raised from a `NodeValue::CodeBlock { fenced: false }` arm, since Lexicon has no legitimate use for indented code; a block falling short by less stays a paragraph and reattaches to an ancestor — `continuation-reattached` (warning), detected by finding the deepest item in the preceding list's open-item chain whose marker column the block passes but whose content column it does not reach. The recommended `4 × (level + 1)` house style sits one column past the content column and must never warn, which is why the test is `marker_col < col < content_col` rather than `col != content_col`. Both catch-all arms in `extract_clause_from_item` and `extract_body` now emit `unsupported-block` rather than dropping unhandled block nodes silently.
- **Ordinal autofix is a reflow, not a regex** — narrowing `100.` to `1.` shrinks the content column by 2, which can push correctly-indented continuation content 4 past it and turn it into a code block. `fix.rs` therefore dedents each item's remaining lines by the same delta, fixes one ordinal per pass and re-parses between passes so positions stay valid, then compares a structural fingerprint (every rendered text block, tagged with clause depth) before and after. A mismatch aborts the whole fix. This is why `--fix` refuses on a document that already has `continuation-indent` findings — the rewrite would genuinely relocate content.
- **Lint filtering pipeline** — one pipeline (`apply_filters` in `src/lint.rs`) serves lint and build: per-rule severity overrides first, then inline suppressions, then ignore lists. Sources: `[lint]` in style.toml (`ignore` + `severity`, parsed into `style::LintSettings`), CLI flags (`--ignore`, `--min-severity`, applied on top), and inline comments (`<!-- lexicon-ignore: code -->` matches diagnostics on the same line or the line below; `<!-- lexicon-ignore-file: code -->` is file-wide). Suppressions that match nothing produce `unused-suppression` (info, lint only). `build` runs the same filters via `lint::filter_build_diagnostics` so build and lint never disagree. `parse-error`/`io-error`/`style-error` are unfilterable; error-default rules cannot be demoted below warning (`invalid-lint-config`); suppression-machinery diagnostics are exempt from inline suppression; directives inside code spans/fences/front-matter are inert. Diagnostics anchor to the containing clause/addendum start line; tables are not scanned (out of spec scope). `lint` resolves style.toml per input file and accepts multiple inputs; JSON output carries `version` (`lint::REPORT_VERSION`, currently 1), single-file reports are flat, multi-file reports nest under `files`. `--format github` emits workflow-command annotations.
