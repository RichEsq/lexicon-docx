...
- **Numbering conventions** — `ClauseNumber` stores only u32 indices (convention-agnostic). Formatting is applied at `full_reference()`/`formatted()` time via a `NumberingConvention` enum (Commonwealth, Decimal, UsTraditional). Word numbering levels (`render/numbering.rs`) are built from a convention-specific data table. The convention is configurable via `numbering_convention` in style TOML and `--numbering-convention` CLI flag.
- **Addendum anchors and structured resolutions** — the anchor map in `resolve.rs` stores `AnchorResolution { value, kind, section }` rather than bare strings (mirroring lexicon-web's `resolve.ts`). Kinds: `BodyClause` → "clause 1.1(a)", `RecitalClause` → "Recital 1", `SectionHeading` → the label itself ("Addendum 1"), `AddendumClause` → "Addendum 1, clause 2.1(a)". Addendum clause lists are numbered in a separate pass with an addendum-local counter (restarts at 1 per addendum, continues across one addendum's clause lists — spec 8.1.2: addenda don't continue the body's numbering). All anchors live in one document-wide map, so cross-references resolve in every direction (body ↔ addendum, addendum-internal). `Addendum::label()` is "Addendum {n}", falling back to the title for an unnumbered addendum (parsed addenda are always auto-numbered per spec 8.1.1).
